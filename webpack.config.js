...
module.exports = {
    ...
    entry: [
        ...process.env.NODE_ENV === 'production' ? ['babel-polyfill'] : [`webpack-dev-server/client?${process.env.WEBPACK_URL || `http://localhost:${process.env.DEV_PORT || 3001}`}/`, 'webpack/hot/only-dev-server', 'babel-polyfill'],
        './scripts/index.ts'
    ],
    output: process.env.NODE_ENV === 'development' ? {
        filename: 'bin/dev/app.js',
        publicPath: (process.env.WEBPACK_URL || `http://localhost:${process.env.DEV_PORT || 3001}`) + '/'
    } : {
        filename: 'app.js',
